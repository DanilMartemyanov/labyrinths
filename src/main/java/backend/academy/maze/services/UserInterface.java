        ...
        while (true) {
            ...
            if (AnswerType.YES.answer().equals(ready)) {
                ...
                while (AnswerType.YES.answer().equals(answer)) {
                    ...
                    printStream.println("Укажите конечную точку, где изображено: " + CellType.PASSAGE.value());
                    printStream.println("Начальная точка \"A\":  ");
                    Coordinate startPoint = CheckInputUser.checkEndPoint(bufferedReader, printStream, maze);
                    printStream.println("Конечная точка \"B\":  ");
                    Coordinate endPoint = CheckInputUser.checkEndPoint(bufferedReader, printStream, maze);
